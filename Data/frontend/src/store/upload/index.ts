...
export const upload = createReducer<Typings.UploadState, Typings.Actions>(INITIAL_STATE, {
...
function removeUpload( state: Typings.UploadState = INITIAL_STATE, action: Typings.RemoveUploadAction): Typings.UploadState {
  ...
  return {
    uploads: state.uploads.filter(upload => upload.video.id !== action.payload.id)
  }
}
...
function updateProgress( state: Typings.UploadState = INITIAL_STATE, action: Typings.UpdateProgressAction): Typings.UploadState {
  ...
  const {indexUpload, indexFile} = findIndexAndFile(state, videoId, fileField);
...
function setUploadError( state: Typings.UploadState = INITIAL_STATE, action: Typings.SetUploadErrorsAction): Typings.UploadState {
  const videoId = action.payload.video.id;  
  const fileField = action.payload.fileField;  
  const {indexUpload, indexFile} = findIndexAndFile(state, videoId, fileField);
  if(typeof indexUpload === 'undefined'){
...
function findIndexAndFile(state: Typings.UploadState, videoId, fileField): {indexUpload?, indexFile?}{
